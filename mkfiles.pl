...
readinput: while (1) {
  ...
  while (scalar @objs > 0) {
    ...
    } elsif (($i eq "[G]" or $i eq "[C]" or $i eq "[M]" or $i eq "[Q]" or
...
sub mfval($) {
    ...
    if (grep { $type eq $_ }
	("vc","vcproj","cygwin","borland","lcc","gtk","mpw","nestedvm","osx","wce","qtopia")) {
	    return 1;
	}
...
if (defined $makefiles{'qtopia'}) {
    $mftyp = 'qtopia';
    $dirpfx = &dirpfx($makefiles{'qtopia'}, "/");

    ##-- Qtopia makefile
    open OUT, ">$makefiles{'qtopia'}"; select OUT;
    print
    "# Makefile for $project_name under Qtopia.\n".
    "#\n# This file was created by `mkfiles.pl' from the `Recipe' file.\n".
    "# DO NOT EDIT THIS FILE DIRECTLY; edit Recipe or mkfiles.pl instead.\n".
    "\n".
    ...
    foreach $p (&prognames("Q")) {
      ($prog, $type) = split ",", $p;

      # We're only interested in a program called sgt-puzzles.
      if ($prog eq "sgt-puzzles") {
	print "\nSOURCES=\\\n";
	foreach (@{$programs{$p}}) {
	    s/\[COMBINED\]//;
	    print "    $_.c\\\n" unless $_ eq "qt";
